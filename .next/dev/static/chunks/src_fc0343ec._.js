...
function PackageDetailPage({ params }) {
    ...
    const description = locale === 'fr' ? `L'expérience ultime : ${item.name}. Ce forfait combine nos meilleures activités en une aventure fluide. Profitez de ${item.included?.join(', ')} en une seule fois.` : `The ultimate experience: ${item.name}. This package combines our best activities into a seamless adventure. Enjoy ${item.included?.join(', ')} all in one go.`;
...
//# sourceMappingURL=src_fc0343ec._.js.map